...
```{r}
patient_id = "X_113"
projected_patient = subset(seurat_projected_all_cd8, subset = patient == patient_id)
...
for(i in 1:length(largest_clones)){
  ...
  cells = which(merged_projected_cd8[["cdr3s_pat"]] == ctype)
...
clonalOccupy(seurat_projected_all_cd8,
                    x.axis = "sample", proportion=FALSE,
                    palette = "heat") + 
                    ggtitle("All sample Colonal Occupy/expansaion proportion") + 
                    theme(legend.position = "right", 
                    axis.text.x = element_text(angle = 45, hjust = 1)) 

fig_name =  paste0(plot_dir, "occupied_repertorire_by_all_sample_count.png")
ggsave(fig_name, height = fig.height, width = fig.width, dpi = 300)
